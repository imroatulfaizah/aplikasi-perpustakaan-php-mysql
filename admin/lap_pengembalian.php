...
$pdf->Cell(25.5,0.7,"Laporan Pengembalian Buku Perpustakaan",0,10,'C');
$pdf->ln(1);
$pdf->SetFont('Arial','B',10);
...
$pdf->Cell(4, 0.8, 'Kode Pengembalian', 1, 0, 'C');
$pdf->Cell(7, 0.8, 'Nama', 1, 0, 'C');
$pdf->Cell(4, 0.8, 'Judul Buku', 1, 0, 'C');
$pdf->Cell(3, 0.8, 'Tanggal Pinjam', 1, 0, 'C');
$pdf->Cell(4.5, 0.8, 'Tanggal Kembali', 1, 1, 'C');
$pdf->SetFont('Arial','',10);
$no=1;
$query=mysql_query("select * from tb_pengembalian");
while($lihat=mysql_fetch_array($query)){
	$pdf->Cell(1, 0.8, $no , 1, 0, 'C');
	$pdf->Cell(4, 0.8, $lihat['kode_pengembalian'], 1, 0,'C');
	$pdf->Cell(7, 0.8, $lihat['nama'],1, 0, 'C');
	$pdf->Cell(4, 0.8, $lihat['judul_buku'],1, 0, 'C');
	$pdf->Cell(3, 0.8, $lihat['tanggal_pinjam'], 1, 0,'C');
	$pdf->Cell(4.5, 0.8, $lihat['tanggal_kembali'],1, 1, 'C');
